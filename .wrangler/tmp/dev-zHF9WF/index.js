  ...
  async fetch(request, env3) {
    const response = await routePartykitRequest(request, env3, {
      ...
      onBeforeConnect: /* @__PURE__ */ __name(async (req, lobby) => {
        ...
        try {
          if (lobby.className === "LobbyServer") {
            await verifyConnectToken(token, env3.JWT_SECRET);
          } else if (lobby.className === "ConversationServer") {
...
// .wrangler/tmp/bundle-IIiOnw/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-IIiOnw/middleware-loader.entry.ts
